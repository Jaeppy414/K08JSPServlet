...
속성정리
 ...
 -->
	...
	<!-- 
	if(number%2==0)과 동일한 조건의 if문으로 해당조건의 결과가 result에 저장된다.
	...
	<!-- Java에서는 비교연산자 ==과 equals()는 다르지만, EL에서는 eq를 통해 값에 대한 
	비교와 문자열에 대한 비교 모두를 할 수 있다.
	여기서 사용한 eq는 Java의 compareTo()와 같이 사전순으로 문자열을 비교한다. -->
	<h4>문자열 비교와 else 구문 흉내내기</h4>
	<!-- 첫번째 if문은 false의 결과가 나온다. -->
	<c:if test="${ sting eq 'Java'}" var="result2">
		문자열은 Java입니다. <br />
	</c:if>
	...
	result4 : ${ result4 } <br />
	<!-- test에는 앞뒤에 하나라도 공백이 들어가면 무조건 false를 반환하게 된다. 
	따라서 주의해야 한다. -->
	...
	받은 후 tjoeun/1234인 경우에는 'tjoeun님, 하이룽~'이라고 출력한다.
	만약 틀렸다면 "아이디/비번을 확인하새요"라고 출력한다.
	...
	<!-- 최초 실행시에는 파라미터가 없는 상태이므로 아무런 내용도 출력되면 안된다. -->
	<c:if test="${ not empty param.user }">
		전송된 아이디 : ${param.user } <br />
		전송된 패스워드 : ${param.pass } <br />
		<!-- 
		아이디, 패스워드를 입력했을 때만 일치여부를판단한 후 결과를
		변수에 저장한다.
		if태그는 else구문이 따로 없으므로 not을 통해 두번째 if문을 구성해야 한다.
		 -->
	...
	<%-- 
	<c:if test="${ param.user eq 'tjoeun' and param.pass eq '1234' }" var="result6">
		tjoeun님, 하이룽~ <br />
	</c:if>
