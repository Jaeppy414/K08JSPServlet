...
try{
	...
	*/
	...
	//파일명에서 확장자 앞의 .(닷)을 찾아 인덱스를 확인한 후 확장자를 잘라낸다.
	//확장자는 파일의 용도를 나타내는 부분이므로 중요하다.
	//파일명에 .(닷)을 여러개 사용할 수 있으므로 끝에서부터 찾는다. ex) "2021년.가을하늘.jpg"
	String ext = fileName.substring(fileName.lastIndexOf("."));
	//현재날짜와 시간 및 밀리세컨즈까지 이용해서 파일명을 만든다.
	String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());
	//확장자와 파일명을 합쳐서 서버에 저장할 파일명을 만들어준다.
	String newFileName = now + ext;
	
	/* 
	서버에 저장된 파일의 파일명을새로운 파일명으로 변경해준다.
	여기서 사용된 separator는 경로를 표시할 때사용하는 구분기호로
	...
	*/
	...
	/* 파일을 제외한 나머지 폼값을 받는다. 단 이때 request내장객체를 통해서가 아니라 mr객체를 통해 
	받아야 하므로 주의해야 한다. 객체는 다르지만 폼값을 받기위한 메서드명은 동일하다.*/
	...
	dto.setName(name);
	dto.setTitle(title);
	//입력 전 StringBuffer객체를 String으로 변환해야 한다.
