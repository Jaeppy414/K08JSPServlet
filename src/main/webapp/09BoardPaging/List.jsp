...
/**페이징 코드 추가부분 s***************/
//web.xml에 설정한 컨텍스트 초기화 파라미터를 읽어와서 산술연산을 위해 정수(int)로 변환한다.
...
가령 게시물의 갯수가 51개라면 나눴을 때의 결과가 5.1이 된다. 이 때 무조건
올림처리하여 6페이지가 나오게 된다.
만약 totalCount를 double로 형변환하지 않으면 정수의 결과가 나오게 되므로
6페이지가 아니라 5페이지라는 결과가 나오게 되므로 주의해야 한다.
*/
...
List.jsp?pageNum=   => 이와 같이 파라미터는 있는데 값이 없을 때는 빈값으로
...
/*
게시물의 구간을 계산한다.
각 페이지의시작번호와 종료번호를 현재페이지번호와 페이지사이즈를 통해
    ...
    for (BoardDTO dto : boardLists)
    {
		...
		//totalCount를사용하여 가상번호를 부여한다.
       	...
       	//현재 페이지번호를 적용한 가상번호 계산하기
       			//전체게시물 수 - (((현재페이지 - 1) * 한페이지출력갯수) + countNum증가치);
